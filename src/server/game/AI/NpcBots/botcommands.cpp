...
public:
	...
	static bool HandleNpcBotAddEntryCommand(ChatHandler* handler, const char* args)
	{
		...
		if (!Info)
		{
			handler->PSendSysMessage("小伙伴的编号错误 请参考如下80001 - 80248");
			handler->SetSentErrorMessage(true);
			return false;
		}
		...
		{
			handler->PSendSysMessage("你必须达到要求的等级才能召唤死亡骑士小伙伴");
			handler->SetSentErrorMessage(true);
			return false;
		}
		...
		{
			if (owner->IsInCombat())
				handler->PSendSysMessage("成功创建小伙伴 将在战斗结束后出现");
			else
				handler->PSendSysMessage("成功创建小伙伴");

			handler->SetSentErrorMessage(true);
			return true;
		}

		handler->PSendSysMessage("无法创建小伙伴");
		handler->SetSentErrorMessage(true);
		return false;
	}
