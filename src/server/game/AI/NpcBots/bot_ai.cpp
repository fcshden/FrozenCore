...
void bot_minion_ai::RezGroup(uint32 REZZ, Player* gPlayer)
{
    ...
    if (!pGroup)
    {
        ...
        {
            me->MonsterWhisper(("正在复活你　"), master);
            rezz_cd = 60;
        }
        return;
    }
    for (GroupReference* itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        if (doCast(target, REZZ))//rezzing it
        {
            me->MonsterWhisper(("正在复活你　"), tPlayer);
            if (tPlayer != master)
            {
...
void bot_ai::_listAuras(Player* player, Unit* unit) const
{
    ...
    else if (unit->GetTypeId() == TYPEID_UNIT)
    {
        ...
        else if (unit->ToCreature()->GetIAmABotsPet())
        {
            ...
            std::string const& ownername = owner ? owner->GetName() : ("无　");
            std::string const& creownername = creowner ? creowner->GetName() : ("无　");
            botstring << ("小伙伴的宠物　, 主人　: ");
            botstring << ownername;
            botstring << (", 小伙伴的主人　: ");
	...
	ch.PSendSysMessage(sCustomMgr->Format(("%s的属性列表　, %s"), unit->GetName().c_str(), botstring.str().c_str()).c_str());
	...
	if (unit == me)
    {
		...
		ch.PSendSysMessage(sCustomMgr->Format(("护甲: %u"), unit->GetArmor()).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("暴击: %0.1f"), unit->GetUnitCriticalChance(BASE_ATTACK, me)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("躲闪: %0.1f"), unit->GetUnitDodgeChance()).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("招架: %0.1f"), unit->GetUnitParryChance()).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("格挡: %0.1f"), unit->GetUnitBlockChance()).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("格挡值: %u"), unit->GetShieldBlockValue()).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("近战攻击强度: %0.1f"), unit->GetTotalAttackPowerValue(BASE_ATTACK)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("远程攻击强度: %0.1f"), unit->GetTotalAttackPowerValue(RANGED_ATTACK)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("普通伤害加成: %0.3f"), unit->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, SPELL_SCHOOL_MASK_NORMAL)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("法术伤害加成: %0.3f"), unit->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, SPELL_SCHOOL_MASK_MAGIC)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("主手伤害范围: 最小: %0.1f, 最大: %0.1f"), unit->GetFloatValue(UNIT_FIELD_MINDAMAGE), unit->GetFloatValue(UNIT_FIELD_MAXDAMAGE)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("副手伤害范围: 最小: %0.1f, 最大: %0.1f"), unit->GetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE), unit->GetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("远程伤害范围: 最小: %0.1f, 最大: %0.1f"), unit->GetFloatValue(UNIT_FIELD_MINRANGEDDAMAGE), unit->GetFloatValue(UNIT_FIELD_MAXRANGEDDAMAGE)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("主手伤害加成: %0.3f"), unit->GetModifierValue(UNIT_MOD_DAMAGE_MAINHAND, BASE_PCT)*unit->GetModifierValue(UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("副手伤害加成: %0.3f"), unit->GetModifierValue(UNIT_MOD_DAMAGE_OFFHAND, BASE_PCT)*unit->GetModifierValue(UNIT_MOD_DAMAGE_OFFHAND, TOTAL_PCT)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("远程伤害加成: %0.3f"), unit->GetModifierValue(UNIT_MOD_DAMAGE_RANGED, BASE_PCT)*unit->GetModifierValue(UNIT_MOD_DAMAGE_RANGED, TOTAL_PCT)).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("主手攻击速度: %0.2f"), float(unit->GetAttackTime(BASE_ATTACK)) / 1000.f).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("副手攻击速度: %0.2f"), float(unit->GetAttackTime(OFF_ATTACK)) / 1000.f).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("远程攻击速度: %0.2f"), float(unit->GetAttackTime(RANGED_ATTACK)) / 1000.f).c_str());
		if (unit == me)
			ch.PSendSysMessage(sCustomMgr->Format(("近战伤害倍率: %0.3f"), dmgmult_melee).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("基础生命值: %u"), unit->GetCreateHealth()).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("当前生命值: %u"), unit->GetMaxHealth()).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("基础法力值: %u"), unit->GetCreateMana()).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("当前法力值: %u"), unit->GetMaxPower(POWER_MANA)).c_str());
		...
		//ch.PSendSysMessage(Format(("小伙伴控制状态　: %s", m_botCommandState == COMMAND_FOLLOW ? "跟随　" : m_botCommandState == COMMAND_ATTACK ? "攻击　" : m_botCommandState == COMMAND_STAY ? "停留　" : m_botCommandState == COMMAND_ABANDON ? "重置　" : "无　")).c_str());
		ch.PSendSysMessage(sCustomMgr->Format(("跟随距离　: %u"), master->GetBotFollowDist()).c_str());
    }
}
//SetStats
...
bool bot_minion_ai::OnGossipHello(Player* player, Creature* creature)
{
    ...
    {
		//player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, ("|cFFFF0000(物品存储出现BUG,修复前请勿将强化过或者放置宝石的装备给BB佩戴-紧急修复中)|r　"), GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 1);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, ("装备管理　"), GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 1);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, ("职责管理　"), GOSSIP_SENDER_ROLES, GOSSIP_ACTION_INFO_DEF + 1);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, ("使用技能　"), GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF + 1);
    }
    ...
    switch (creature->GetBotClass())
    {
        case CLASS_MAGE:
            ...
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, ("我需要喝水　"), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
            break;
        default:
            break;
    }
    ...
    case 7000: //stat
    {
        ...
        case GOSSIP_SENDER_CLASS: //food/drink (classes: MAGE)
        {
            ...
            if (result != SPELL_CAST_OK)
            {
                ...
                delete foodspell;
                me->MonsterWhisper(("我现在不能这么做。　"), player);
            }
            else
            {
                aftercastTargetGuid = player->GetGUID();
                foodspell->prepare(&targets);
                me->MonsterWhisper(("给你的。　"), player);
            }
            break;
        }
        case GOSSIP_SENDER_EQUIPMENT: //equips change s1: send what slots we can use
        {
            ...
            //general
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, ("查看你的装备　"), GOSSIP_SENDER_EQUIPMENT_LIST, GOSSIP_ACTION_INFO_DEF + 1);
            ...
            for (uint8 i = 0; i != BOT_INVENTORY_SIZE; ++i)
            {
                if (uint32 itemId = master->GetBotEquip(me, i))
                {
                    ...
                    if (ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itemId))
                        ...
                        msg << ("未知物品　");
                    msg << ("在栏位　") << uint32(i) << " (" << _getNameForSlot(i + 1) << ')';
                    if (i < BOT_SLOT_RANGED && einfo && einfo->ItemEntry[i] == itemId)
                        msg << ("是　|cffe6cc80|h[　初始装备　]|h|r");
        ...
        case GOSSIP_SENDER_EQUIPMENT_SHOW: //equips change s2: send list of equippable items
        {
            ...
            if (action - GOSSIP_ACTION_INFO_DEF <= BOT_SLOT_RANGED)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, ("还原初始装备　"), GOSSIP_SENDER_EQUIP_RESET, action);

            //s2.2.2 add unequip option if have weapon
            if (master->GetBotEquip(me, action - (GOSSIP_ACTION_INFO_DEF + 1)))
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, ("卸下装备　"), GOSSIP_SENDER_UNEQUIP, action);

            //s2.2.3a: add an empty submenu with info if no items are found
            if (itemList.empty())
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, ("那个……我没有任何东西可以给你　"), 0, GOSSIP_ACTION_INFO_DEF + 1);
            }
            else
            {
        ...
        case GOSSIP_SENDER_UNEQUIP: //equips change s3: Unequip
        {
            if (action - GOSSIP_ACTION_INFO_DEF == BOT_SLOT_MAINHAND)
            {
                ...
                {
                    me->MonsterSay(("什么，你让我赤手空拳？没门！　"), LANG_UNIVERSAL, player);
                    break;
                }
            }
            else if (action - GOSSIP_ACTION_INFO_DEF == BOT_SLOT_OFFHAND)
            {
                ...
                {
                    me->MonsterSay(("不会吧。　"), LANG_UNIVERSAL, player);
                    break;
                }
            }
            else if (action - GOSSIP_ACTION_INFO_DEF == BOT_SLOT_RANGED)
            {
                if (me->GetBotClass() == CLASS_HUNTER)
                {
                    me->MonsterSay(("耶！非常有趣。　"), LANG_UNIVERSAL, player);
                    break;
                }
            }
            if (action - (GOSSIP_ACTION_INFO_DEF + 1) < BOT_SLOT_RANGED)
            {
                ...
                {
					me->MonsterSay(("请问在你心里我的武器和我相比是不是更具魅力呢？　"), LANG_UNIVERSAL, player);
                    break;
                }
            }

            if (Unequip(action - (GOSSIP_ACTION_INFO_DEF + 1)))
                me->MonsterSay(("呃……　"), LANG_UNIVERSAL, player);
            break;
        }
...
void bot_minion_ai::SummonBotsPet(uint32 entry)
{
    ...
    {
        //annoy master
        me->MonsterWhisper(("为什么我要尝试着召唤一只未知的宠物！？　"), master);
        return;
    }
    uint32 armor = 0;
    float x(0),y(0),z(0);
    ...
    if (!m_botsPet)
    {
        me->MonsterWhisper(("召唤宠物失败！　"), master);
        return;
    }
...
bool bot_minion_ai::Equip(uint32 itemId, uint8 slot)
{
    ...
    if (slot < BOT_SLOT_RANGED)
    {
        ...
        if (me->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + slot))
        {
            if (!Unequip(slot))
            {
                me->MonsterSay("你没有足够的背包空间来存放我的装备。　", LANG_UNIVERSAL, master);
                return false;
            }
        }
